...
class MultitaskBERT(nn.Module):
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        logits = self.linear(self.dropout(pooled_rep))
        return logits
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = loss_sentiment + loss_similarity + loss_paraphrase
...
# what is my objective? to minimize total losses
...
def hyperparameter_tune(args):
    ...
    best_parameters, values, experiment, model = optimize(
        parameters=[
            ...
            {"name": "epochs", "type": "range", "bounds": [0, 20]},
            #{"name": "max_epoch", "type": "range", "bounds": [1, 30]},
            #{"name": "stepsize", "type": "range", "bounds": [20, 40]},
        ],
      
        evaluation_function=train_multitask(args),
