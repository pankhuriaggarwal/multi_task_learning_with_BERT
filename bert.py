...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    ### TODO
    bs, num_attention_heads, seq_len, head_size = query.size()
    k = key.transpose(2,3)
    S = query @ k
    S = S + attention_mask
    S = S * (1/math.sqrt(head_size))
    S = F.softmax(S,dim=-1)
    V = S @ value
    V = V.transpose(1,2)
    V = V.reshape(bs, seq_len, -1)
    # returning shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    return V
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    addnorm1 = self.add_norm(hidden_states,att,self.attention_dense,self.attention_dropout,self.attention_layer_norm)
    ff = self.interm_af(self.interm_dense(addnorm1))
    addnorm2 = self.add_norm(addnorm1,ff,self.out_dense,self.out_dropout,self.out_layer_norm)
    return addnorm2
