...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dropout_sts = torch.nn.Dropout(0.5)
        self.dropout_sst = torch.nn.Dropout(0.5)# higher dropout for sts to combat overfitting
        ...
        torch.nn.Linear(2*BERT_HIDDEN_SIZE, BERT_HIDDEN_SIZE),
        #torch.nn.LeakyReLU(),
        torch.nn.ReLU(),
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        logits = self.linear_sentiment(self.dropout_sst(pooled_rep))
        return logits
...
## Changing to train on all data for all datasets (separate for-loops)
def train_multitask(args):
    ...
    if reg == 'l2':
        optimizer = PCGrad( AdamW(model.parameters(), lr=lr, weight_decay=0.05) )
    ...
    for epoch in range(args.epochs):
        ...
        desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            # class_weights=torch.FloatTensor([3.22, 1.59, 2.17, 1.51, 2.73 ]).to(device)#w0, w1...order
            # loss_sentiment = F.cross_entropy(logits_sst, bs_labels.view(-1), reduction='sum' , weight = class_weights) / args.batch_size
