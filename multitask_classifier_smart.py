...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dropout_sst = torch.nn.Dropout(0.5)
        self.linear = torch.nn.Linear(BERT_HIDDEN_SIZE, N_SENTIMENT_CLASSES)
        ...
        self.linear2 = torch.nn.Linear(2*BERT_HIDDEN_SIZE, 1)
        self.linear3 = torch.nn.Linear(2*BERT_HIDDEN_SIZE, 1)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        logits = self.linear(self.dropout_sst(pooled_rep))
        return logits
        
    def predict_sentiment_for_smart(self, input_ids, attention_mask):
        '''Given a batch of sentences, outputs logits for classifying sentiment.
        There are 5 sentiment classes:
        (0 - negative, 1- somewhat negative, 2- neutral, 3- somewhat positive, 4- positive)
        Thus, your output should contain 5 logits for each sentence.
    ...
    def predict_paraphrase(self,
        ...
        logits = self.linear2(self.dropout(final_embed))
        return logits
    ...
    def predict_similarity(self,
        ...
        '''
        ### TODO
        input_ids_1, attention_mask_1, input_ids_2, attention_mask_2 = args[:]
        pooled_rep_1 = self.forward(input_ids_1, attention_mask_1)
        pooled_rep_2 = self.forward(input_ids_2, attention_mask_2)
        # combine embeddings
        final_embed = torch.cat((pooled_rep_1, pooled_rep_2), dim=1)
        logits = self.linear3(self.dropout_sts(final_embed))
        return logits
        #raise NotImplementedError
    def predict_similarity_for_smart(self, args):
        '''Given a batch of pairs of sentences, outputs a single logit corresponding to how similar they are.
        Note that your output should be unnormalized (a logit).
        '''
        ### TODO
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            if reg == 'smart':
                # loss_fn = lambda x: F.mse_loss(x, bt_labels.float().view(-1), reduction='sum') / args.batch_size
                loss_fn = nn.CrossEntropyLoss()
                embed = model.predict_sentiment_for_smart(bs_ids, bs_mask)
                embed = embed.to(device)
                eval_fn = torch.nn.Linear(BERT_HIDDEN_SIZE, N_SENTIMENT_CLASSES)
                eval_fn = eval_fn.to(device)
                #embed = torch.rand(1, 10) # [batch_size, in_features]
                regularizer = SMARTLoss(eval_fn, loss_fn)
                state = eval_fn(embed) # [batch_size, out_featueres]
                state = state.to(device)
                # Compute regularation loss
                loss = regularizer(embed, state)
                loss_sentiment = loss
                #print("loss sentiment", loss_sentiment)

            # add regularization
            if reg == 'l1':
                norm = sum([p.pow(2).sum() for p in model.parameters()])
                loss_similarity += 0.05 * norm
            elif reg == 'l2':
                norm = sum([p.abs().sum() for p in model.parameters()])
                loss_similarity += 0.05 * norm
            elif reg == 'linf':
                norm = max([p.abs().max() for p in model.parameters()])
                loss_similarity += 0.05 * norm
...
def get_args():
    ...
    parser.add_argument("--lr", type=float, help="learning rate, default lr for 'pretrain': 1e-3, 'finetune': 1e-5",
                        default=1e-5)
    parser.add_argument("--regularization", type=str, help="regularization, default l2",
                        default='l2')
